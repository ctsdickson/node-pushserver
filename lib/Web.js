...
function sendNotifications(notifs) {
    ...
    notifs.forEach(function (notif) {
        var users = notif.users,
            fcmPayload = notif.fcm,
            androidPayload = notif.android,
            iosPayload = notif.ios,
            baiduPayload = notif.baidu,
            getuiPayload = notif.getui,
            pushyPayload = notif.pushy,
            target;

        if (fcmPayload && androidPayload && iosPayload) {
            target = 'all'
        } else if (iosPayload) {
            target = 'ios'
            ...
            target = 'fcm';
        } else if (baiduPayload) {
            target = 'baidu';
        } else if (getuiPayload) {
            target = 'getui';
        } else if (pushyPayload) {
            target = 'pushy';
        }
        ...
        var fetchUsers = users ? pushAssociations.getForUsers : pushAssociations.getAll,
            callback = function (err, pushAssociations) {
                ...
                push.send(pushAssociations, fcmPayload, androidPayload, iosPayload,baiduPayload,getuiPayload,pushyPayload);
            },
...
function validateNotification(notif) {
    ...
    valid = valid && (!!notif.fcm || !!notif.ios || !!notif.android);
